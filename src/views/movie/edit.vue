...
export default {
  ...
  created() {
    ...
    this.getMovieDataById(this.id)
    this.getCityList()
  },
  methods: {

    getMovieDataById(id) {
      axios.get('getMovieDataById/' + id).then(res => {
        // console.log(res)
        this.movie.title = res.data.data.title
        this.movie.stars = res.data.data.stars
        this.movie.score = res.data.data.score
        this.movie.region = res.data.data.p._id
